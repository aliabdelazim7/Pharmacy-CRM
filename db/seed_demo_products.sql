-- ─────────────────────────────────────────────────────────────
-- Demo seed: realistic pharmacy products for client demos.
-- Run ONCE in Supabase → SQL Editor → New query → Run.
-- Idempotent: safe to re-run (categories guarded, products keyed by barcode).
-- Prices are in EGP. Ordering: purchase < wholesale < half_wholesale < sale.
-- ─────────────────────────────────────────────────────────────

-- 1) Categories — insert only if missing (no unique constraint on name).
...
-- 2) Products — barcode is unique, so re-runs skip existing rows.
...
from (values
  -- name, barcode, purchase, wholesale, half, sale, stock, unit, category
  ...
  ('ديكلوفيناك جل 50جم',               '6221000000059',  25,  30,  33,  40,  60, 'علبة',  'مسكنات وخافض حرارة'),
  ...
  ('أوتريفين نقط للأنف',               '6221000000141',  20,  24,  27,  33,  90, 'زجاجة', 'برد وإنفلونزا وسعال'),
  ...
  ('فوار إينو',                        '6221000000257',   6,   8,   9,  12, 250, 'علبة',  'جهاز هضمي'),

  ('سيتافيل غسول 250مل',               '6221000000264', 130, 150, 165, 190,  45, 'علبة',  'عناية بالبشرة والشعر'),
  ('بانثينول كريم 30جم',               '6221000000271',  25,  30,  34,  42,  90, 'علبة',  'عناية بالبشرة والشعر'),
  ('فيشي واقي شمس SPF50',              '6221000000288', 220, 250, 270, 310,  35, 'علبة',  'عناية بالبشرة والشعر'),
  ...
  ('سيباميد بيبي غسول 250مل',          '6221000000400',  90, 105, 115, 135,  50, 'علبة',  'عناية بالأم والطفل')
